...
#' Estimating merchantable tree volume from DBH and height for a list of trees in plots
#'
#' @description Estimate merchantable tree volume in dm3 with Fortin et al. (2007) equation for a list of trees in plots in a dataframe.
...
#'
#' @param fic_arbres Dataframe containing a list of trees in plots, these informations must be in the dataframe:
#' \itemize{
...
#' @param iteration If \code{mode_simul}='STO', iteration number to estimate (default 1)
...
#' @return Dataframe \code{fic_arbres} with column vol_dm3 with estimated volume in dm3 (volume for one tree)
#' @export
#'
#' @examples
